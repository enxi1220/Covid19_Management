...
#define PREV_DEATH 2
#define PREV_RECOVER 10
#define PREV_UNDER_TREATMENT 88
#define PREV_DEATH_RATE 2.00
#define PREV_RECOVER_RATE 10.00
#define PREV_TREATMENT_RATE 88.00
...
int highestDay[400] = { PREV_DAY };
int highestDayCount = 1;
int lowestCases = PREV_CASE;
int lowestDay[400] = { PREV_DAY };
int lowestDayCount = 1;
...
void getHighestLowest()
{
	...
	else if (currCase == highestCases)//maybe multiple days have the same cases
	{
		highestDay[highestDayCount] = day;
		++highestDayCount;
	}
	...
	else if (currCase == lowestCases)//maybe multiple days have the same cases
	{
		lowestDay[lowestDayCount] = day;
		++lowestDayCount;
	}
}
...
void conclusion()
{
	...
	printf("\t\t\t=     %-25s     => %5d  [  %s %4d  ]            = \n", "HIGHEST number of cases", highestCases, " Day", highestDay[0]);
	for (int i = 1; i < highestDayCount; i++)
	{
		printf("\t\t\t=                                             [  %s %4d  ]            =\n", " Day", highestDay[i]);
	}

	printf("\t\t\t=     %-25s     => %5d  [  %s %4d  ]            = \n", "LOWEST number of cases", lowestCases, " Day", lowestDay[0]);
	for (int i = 1; i < lowestDayCount; i++)
	{
		printf("\t\t\t=                                             [  %s %4d  ]            =\n", " Day", lowestDay[i]);
	}
	printf("\t\t\t=                                                                        =\n");
